...
static u32 send_interval_ms = 5; // 发送间隔：每5ms发送一次
...
void test_data_send_packet(void)
{
    ...
    //     {
    //         // 判断计数器是否为奇数，奇数发送，偶数不发送
    //         if (current_ble_counter % 2 == 1)
    //         {
    //             // 发送 "Hera + 数字" 数据
    ...
    //         else
    //         {
    //             // 偶数不发送，只更新时间避免卡在time_after判断
    //             last_send_time = current_time;
    //             log_info("BLE sending disabled (counter=%d is even)\n", current_ble_counter);
    //         }
    //     }
    // }
    // //================= 修改结束 =================
...
//	Master bat
    ...
    // Box bat
    if ((master_charge && (master_bat != 0)) || (slave_charge && (slave_bat != 0)))
    {
        // earphone in charge
#if TCFG_CHARGESTORE_ENABLE
    ...
    // check if change
    u8 tchange = 0;
    if ((tbat_charge_L != __this->bat_charge_L) || (tbat_charge_R != __this->bat_charge_R) || (tbat_charge_C != __this->bat_charge_C) || (tbat_percent_L != __this->bat_percent_L) || (tbat_percent_R != __this->bat_percent_R) || (tbat_percent_C != __this->bat_percent_C))
    {
        tchange = 1;
    }
...
static void deal_sibling_seq_rand_sync(void *data, u16 len, bool rx)
{
    ...
    if (rx)
    {
        ...
        case TWS_VERSON_INFO:
            ...
            if ((cur_ver_info > sibling_ver_info) && (tws_api_get_role() == TWS_ROLE_MASTER))
            { // 如果主机版本号比从机高需要进行role_switch
            ...
            else if (cur_ver_info == sibling_ver_info)
            { // 如果tws连接同步版本号一致需要把标志清楚防止重复升级
                tws_need_update = 0;
